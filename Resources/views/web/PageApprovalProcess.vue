<template>
	<NewbieTable ref="list" :url="route('api.manager.approval.process.items')" :columns="columns()" row-key="id">
		<template #functional>
			<NewbieButton type="primary" icon="PlusOutlined" @click="onEdit(false)">新增审批流程</NewbieButton>
		</template>
	</NewbieTable>
				...
				<a-step>
					<template #subTitle>
						<a-avatar :size="64" @click="onOpenNodeEditor()">
							<NewbieIcon icon="PlusOutlined" style="font-size: 28px"></NewbieIcon>
						</a-avatar>
						<br />
						<span>添加审批节点</span>
					</template>
	...
	<a-modal title="添加审批节点" v-model:visible="state.showNodeEditor" :width="800" :footer="null">
...
const onBeforeSubmit = ({ rawForm }) => {
	state.processForm = { ...state.processForm, ...rawForm }
