...
class ApprovalController extends BaseManagerController
{
    ...
    {
        $approval_types = array_values(config('module.Approval.approval_types'));
        ...
        $approver_options = [
            ['label' => '指定部门', 'value' => Department::class],
            ['label' => '指定角色', 'value' => Role::class],
            ['label' => '指定用户', 'value' => User::class],
        ];

        $subsequent_action_options = [
            ['label' => '不可见', 'value' => ApprovalSubsequentAction::Invisible->value],
            ['label' => '可见不可审批', 'value' => ApprovalSubsequentAction::Visible->value],
            ['label' => '可审批', 'value' => ApprovalSubsequentAction::Approve->value]
        ];
    ...
    public function processEdit(Request $request)
    {
        list($input, $error) = land_form_validate(
            ...
            [
                ...
                'nodes.*.approver_id' => 'bail|required|integer',
    ...
    public function approve(Request $request, ApprovalService $service)
    {
        ...
        $table_name = $task->approvable->getTable();
        $exist_snapshot = Schema::hasTable($table_name . '_snapshot');

        list($result, $error) = $service->approve(auth()->user(), $task->approvable, $process, $input['approval_status'], $input['approval_comment'] ?? '', $exist_snapshot);
        ...
        if (in_array($result['approval_status'], [ApprovalStatus::Approved->value, ApprovalStatus::Rejected->value])) {
